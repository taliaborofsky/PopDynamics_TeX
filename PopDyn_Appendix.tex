...
We first prove part (i). From (\ref{sys_allB}a), $W - \delta = 1$ because $\hN_p > 0$. Then from Eqn. \eqref{before_delta_eq}, $\delta = R$ is only possible if $L(\hu_r,\hr) = 0$ or $\hr = R$. If $L(\hu_r,\hr) = 0$, assuming $\pc > 0$, $\hr = K\hu_r = 0$.
...
L(\hu_r, \hr) = K \frac{(\delta - R)(1+r)}{(r-R)(1+\delta)} + \pc \frac{r}{r+R} = \frac{\delta - R}{r-R}.
\end{align}
...
Given $\pc > 0$, if $\hr = 0$ then at equilibrium, from \eqref{L_u}, $W u_r= K u_r$, so either $W = K$ or $\hu_r = 0$. Say $W = K$ and $\hu_r \neq 0$. Then from \eqref{W_allB},
$$
K = 1 + R(1-K u_r)
$$
which is a contradiction because $0 \leq K < 1$, leaving us with $\hu_r = \hr = 0$. 

Next, if $\hu_r = \hr = 0$, then $\hN_p = 0$ if $R < \delta$. The mean population fitness is $W = 1 + R$ because $\hu_r = \hr = 0$. Since $R < \delta$, $W \neq 1 + \delta$, and thus the only possible equilibrium is $\hN_p = 0$.
...
\end{align}
Thus $\hu_r$ is the positive root of $Q_u(u_r)$ because $K(1-R)>0$, $Q_u(0) < 0$, and
$$
Q_u(1) = (1+R)(1-K) - \pc > R \pc > 0.
$$
...
\subsection{Internal Stability, no time delay}

The CP density near equilibrium is
\begin{align*}
